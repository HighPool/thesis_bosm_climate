...
"""
Multirun experiment metódy Random Search na úlohe windwake.

Skript:
- načíta jednu windwake konfiguráciu,
- vykoná viac behov Random Search,
- agreguje metriky cez behy,
- uloží per-problem JSON,
- uloží summary JSON,
- vytvorí CSV pre IOHanalyzer.
"""
...
def main():
    ...
    base_out_dir = Path("results/wind/final") / experiment_tag / "random_search"
    ...
    summary_path = (
        base_out_dir
        / f"random_search_summary_wind_n{n_turbines}_budget{budget}_runs{n_runs}.json"
    )
    csv_path = (
        base_out_dir
